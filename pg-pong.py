"""
    ...
    Uses Gymnasium's ALE/Pong-v5 environment, instead of OpenAI's gym Pong-v0.
    More information about Pong environment here: 
    https://gymnasium.farama.org/environments/atari/pong/

    The code is based on Andrej Karpathy's blog post:
...
render = False
save_checkpoint_interval = 100 # default 100

# model initialization
D = 80 * 80  # input dimensionality: 80x80 grid
if resume:
    logging.info("Resuming from previous checkpoint")
    model = pickle.load(open("ckpt/save.p", "rb"))
else:
    model = {}
...
def prepro(I):
    """prepro 210x160x3 uint8 frame into 6400 (80x80) 1D float64 vector"""
    I = I[35:195]  # crop
...
while True:
    ...
    if terminated or truncated:
        ...
        if episode_number % save_checkpoint_interval == 0:
            logging.info(f"Saving checkpoint at episode {episode_number}")
            pickle.dump(model, open("./ckpt/save.p", "wb"))
        reward_sum = 0
